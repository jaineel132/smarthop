            ...
            {step.id < currentStep ? (
              <div className="flex items-center">
                ...
                {stepIdx !== steps.length - 1 && (
                  <div className="ml-4 h-0.5 w-8 bg-teal-700 sm:w-16" />
                )}
              </div>
            ) : step.id === currentStep ? (
              <div className="flex items-center" aria-current="step">
                <span className="relative flex h-10 w-10 items-center justify-center rounded-full border-2 border-teal-700 bg-white">
                  <motion.span
                    layoutId="active-step-ring"
                    className="absolute inset-0 rounded-full border-2 border-teal-700"
                    initial={{ scale: 0.8, opacity: 0 }}
                    animate={{ scale: 1.2, opacity: 0 }}
                    transition={{ repeat: Infinity, duration: 1.5 }}
                  />
                  <span className="text-teal-700 font-semibold">{step.id}</span>
                </span>
                {stepIdx !== steps.length - 1 && (
